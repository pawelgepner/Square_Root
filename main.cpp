...
inline float SQRT_P22f ( float& x ) {
    ...
    if ( i_mx >= 0x00800000 ){
        ...
        k2 = - 0.86150539603051815126646385039843723e-8f ; // 1 , 2
    }
    else
    {
        ...
        k2 = -0.12183586476093894040520123764728955e-7f  ; // 2 , 4
    }
...
inline float SQRT_P42f ( float& x ){
    ...
    if ( i_mx >= 0x00800000 ){
        ...
        k4 = -0.18188157092109794979119076853640167e-22f ; // 1 , 2
    }
    else
    {
        i_exp = ( i_exp +0x3f000000 ) >>1;
        ...
        k4=-0.25721938434234066610403190133237333e-22f;//2,4
...
float Sqrt_linear_2parts_mask(float x){
    float y;
    int i;
    u_int32_t i_m, i_exp;
    int k1, k0;
...
float Sqrt_linear_2parts_mask_last_part_(float x){
    float y;
    int i;
    u_int32_t i_m, i_exp;
    int k1, k0;
...
float SQRT_P28int_ver2 (float& x)
{
    float y;
...
float Sqrt_P3_2parts_mask(float x){
    float y;
...
void calculateExpRMSD(const std::string &desc, F func) {
    ...
    std::cout << std::left
        ...
        << std::setw(17) << std::setprecision(4) << std::fixed<< -log2(maxPoh)<< std::endl;
...
int main() {
    ...
    profileExp("SQRT_P28int", iterationsCount, step, SQRT_P28int_ver2);
    ...
    profileExp("Sqrt_linear_2parts_mask", iterationsCount, step, Sqrt_linear_2parts_mask);
    profileExp("Sqrt_linear_2parts_mask_last_part_", iterationsCount, step, Sqrt_linear_2parts_mask_last_part_);
    profileExp("Sqrt_linear_2parts_frexpf", iterationsCount, step, Sqrt_linear_2parts_frexpf);
    profileExp("Sqrt_P3_2parts_mask", iterationsCount, step, Sqrt_P3_2parts_mask);
    ...
    profileExpCycles("SQRT_P28int", iterationsCount, step, SQRT_P28int_ver2);
    ...
    profileExpCycles("Sqrt_linear_2parts_mask", iterationsCount, step, Sqrt_linear_2parts_mask);
    profileExpCycles("Sqrt_linear_2parts_mask_last_part_", iterationsCount, step, Sqrt_linear_2parts_mask_last_part_);
    profileExpCycles("Sqrt_linear_2parts_frexpf", iterationsCount, step, Sqrt_linear_2parts_frexpf);
    profileExpCycles("Sqrt_P3_2parts_mask", iterationsCount, step, Sqrt_P3_2parts_mask);
    ...
    calculateExpRMSD<float>("SQRT_P28int", SQRT_P28int_ver2);
    ...
    calculateExpRMSD<double>("Sqrt_linear_2parts_mask", Sqrt_linear_2parts_mask);
    calculateExpRMSD<double>("Sqrt_linear_2parts_mask_last_part_", Sqrt_linear_2parts_mask_last_part_);
    calculateExpRMSD<double>("Sqrt_linear_2parts_frexpf", Sqrt_linear_2parts_frexpf);
    calculateExpRMSD<double>("Sqrt_P3_2parts_mask", Sqrt_P3_2parts_mask);
